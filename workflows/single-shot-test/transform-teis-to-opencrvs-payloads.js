fn(state => {
  const HOSPITAL_ID = "c36a0dad-c790-4824-aa62-6186deea5a4b";
  const attr = (attrs, displayName) => attrs.find(a => a.displayName === displayName)?.value;
  ...
  const declarations = state.data.trackedEntities.map(tei => {
    ...
    return {
      'child.name': { firstname: attr(childAttrs, 'First name'), surname: attr(childAttrs, 'Last name') },
      ...
      'mother.name': { firstname: attr(motherAttrs, 'First name'), surname: attr(motherAttrs, 'Last name') },
    };
  });
